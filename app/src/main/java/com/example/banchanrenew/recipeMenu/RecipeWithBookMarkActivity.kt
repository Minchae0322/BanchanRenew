...
class RecipeWithBookMarkActivity: AppCompatActivity() {
    private lateinit var binding: ActivityRecipeBinding
    private var spinnerNum: Int = 0
    private var dishList: List<Dish> = MainActivity.db.recipeDao().getRecipeWithBookMarkList(1)
    ...
    private fun findDishListWithText(charText: String) {
        ...
        when(spinnerNum) {
            ...
            1 -> {
                for(dish in dishList) {
                    var essentialList: MutableList<EssentialIngredients> = MainActivity.db.recipeDao().getEssentialListWhereDishID(dish.recipeID)
    ...
    private fun initSpinner() {
        ...
        binding.spinnerSearch.adapter = ArrayAdapter(applicationContext,
